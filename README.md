...
[lilycalInventory](https://lilxyzw.github.io/lilycalInventory/) の [LI CostumeChanger](https://lilxyzw.github.io/lilycalInventory/ja/docs/components/costumechanger.html) コンポーネントのうち「マテリアルの置き換え」を使っている場合は公式の glTF 拡張である [KHR_materials_variants](https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_materials_variants) に変換します。

* 名前は「メニュー・パラメーター名」とコスチューム側の「メニュー名」を利用
  * `${メニュー・パラメーター名}/${メニュー名}` の法則で名前が設定されます
  * 例えば「メニュー・パラメーター名」が「衣装」で「メニュー名」が「派生」の場合は `衣装/派生` になります
* 「メッシュ」と「置き換え先」は VRM に対応するものと直接マッピング
  * 置き換え先が `None` の場合は元のマテリアルを参照します
* それ以外の項目は全て無視される

> [!CAUTION]
> 切り替えに必要なマテリアル数及びテクスチャ数が増加する関係でその分出力される VRM のファイルサイズが大きくなるため、切り替えが不要の場合は `LI CostumeChanger` コンポーネントを無効にしてください。また `KHR_materials_variants` に対応しているかどうかは読み込み先アプリケーションの実装依存となります
...
`KHR_materials_variants` は仕様として単純なマテリアルの切り替えのみ対応である関係で `LI CostumeChanger` がもつ機能であるオブジェクトのオンオフ、材質のプロパティ操作、アニメーション再生、ブレンドシェイプの切り替えその他諸々には対応していません。それらが設定されている場合でも全て無視されます。
