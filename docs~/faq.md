...
## VRChat SDK / lilToon / Modular Avatar / Avatar Optimizer は必須ですか？
...
* [Modular Avatar](https://modular-avatar.nadena.dev/ja/)
  * 後述の「[プラットフォームフィルター](https://modular-avatar.nadena.dev/ja/docs/reference/platform-filter)」が利用可能になります
  * Modular Avatar を導入すると NDMF も同時に導入されます
...
## VRM 0.x 形式のアバターとして書き出せますか？

書き出せません。また設計方針上 VRM 0.x 形式で書き出す機能を実装する予定もないので「出力した VRM 1.0 アバターを 0.x にダウングレードできますか？」に従いダウングレードしてください。

## 出力した VRM 1.0 形式のアバターを 0.x にダウングレードできますか？

> [!NOTE]
> 具体的には [Warudo](https://docs.warudo.app/jp/docs) と [バーチャルモーションキャプチャー](https://vmc.info) が 2026/3/1 時点で VRM 0.x 形式のみ対応のため、利用する際は VRM 0.x 形式へのダウングレードが必要です
...
* [VRM1.0 to VRChat Converter](https://booth.pm/ja/items/8294490)
* [VRoid Studio から VRM を XAvatar を通じて VRChat アバターに変換](https://vroid.pixiv.help/hc/ja/articles/38728373457561-VRChat%E3%81%A7%E4%BD%BF%E3%81%86%E3%81%AB%E3%81%AF)
...
## NDMF VRM Exporter で生成した VRM ファイルが暗くみえる

シェーダに lilToon 以外が使われている場合は glTF の材質として扱われるため、描画の仕様上暗く見えることがあります。そのため lilToon を使っているかどうかを確認してください。lilToon を使っているにも関わらず問題が残る場合はよくある質問にある次の項目の回答を確認してください。
...
> [!IMPORTANT]
> lilToon 2.x 系統を使っている場合は NDMF 1.0.12 以上であるかを確認してください
...
Modular Avatar の [プラットフォームフィルター](https://modular-avatar.nadena.dev/ja/docs/reference/platform-filter) コンポーネントを表示または非表示にしたいゲームオブジェクトに付与します。このときプラットフォームを `VRM 1.0 (NDMF VRM Exporter)` にし「含める（表示）」または「除外する（非表示）」を設定します。その後に「[導入と使い方のその２](usage.md)」の方法であるアバタープラットフォームから `VRM 1.0 (NDMF VRM Exporter)` を選択して書き出してください。
