...
[VRM Animation](https://vrm.dev/vrma/) 形式への変換は以下の仕様に基づいて行われます。変換元モデルは NDMF Console 経由で `VRM 1.0 (NDMF VRM Exporter)` を選択して出力する際に使われるデータと同一のものが利用されます

* 「出力」ボタンによってのみ行われ、ビルドや NDMF Console のタイミングでは行われない
* 出力時はフォルダ指定により指定されたフォルダに全ての変換対象のアニメーションが書き出される
  * Animation Clip のファイル名に拡張子 `.vrma` を付与する形で書き出し
  * 指定されたフォルダになんらかのファイルまたはフォルダが含まれても警告を行いません
* `VRMC_vrm_animation` のメタデータに変換元の Animation Clip のファイル名と GUID が含まれる
* 変換元モデルのヒューマノイドにマッピングされているノードのみ書き出される
  * それ以外の無関係のノードは出力段階でフィルタリングされ、書き出されません
* 表情のアニメーションは含まれない
