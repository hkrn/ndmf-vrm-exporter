...
* `Ignore`
  * 分岐元が含まれないそれぞれ独立した VRM Spring Bone のジョイント集合が作られます [^1]
* `First`
...
Constraint または VRC Constraint が使われている場合は VRM Constraint に変換されます。またその場合は以下の三種類に対応しています。[^2] 変換元に複数の Constraint または VRC Constraint が存在する場合は最初のひとつのみが変換されます。[^3]
...
`RotationConstraint` は `Freeze Rotation Axes` の結果によって変換先が変わります（✅ はチェックあり、❌ はチェックなしを意味します）
...
> [!IMPORTANT]
> MToon の仕様上 lilToon の見た目と同じように再現出来ない場合があります。特に光沢、ラメ、宝石、屈折、ファーは直接対応する項目がないため正しく変換できません

lilToon シェーダが使われている場合は MToon 互換設定に変換します（MToon 未対応の環境のために `KHR_materials_unlit` を付与するため、未対応の環境でも同一ではないものの近い見た目になります）。その場合は以下の処理を行います。
...
VRoid Studio の XAvatar 形式経由での VRChat アバター向けに VRM 1.0 用の MToon シェーダが使われている場合は変換ではなくパラメータを直接コピーする形で対応します。VRM 0.x 用の MToon シェーダには対応しません。

VRM 1.0 用の MToon または lilToon 以外のシェーダが使われている場合は MToon の変換は行われず、glTF 準拠の最低限の設定で変換します。

### lilToon -> MToon 変換における既知の制約

* メインカラーの 2nd/3rd の UV Mode に UV0 **以外** を指定している場合はテクスチャの焼き込みが行われません [^4]
...
> [!NOTE]
> VRM Animation は変換元アバター以外の別の VRM アバターにも原理的には適用できますが、各アバターの体型及びボーンの位置や想定される回転量の差異から必ずしも全ての VRM アバターが期待通りに再生できるとは限りません。基本的には変換元アバター専用のアニメーションとして扱うのが無難です

[VRM Animation](https://vrm.dev/vrma/) 形式への変換は以下の仕様に基づいて行われます。変換元アバターは NDMF Console 経由で `VRM 1.0 (NDMF VRM Exporter)` を選択して出力する際に使われるデータと同一のものが利用されます

* `Export` ボタンによってのみ行われ、ビルドや NDMF Console のタイミングでは行われない
* 出力時はフォルダ指定により指定されたフォルダに全ての変換対象のアニメーションが書き出される
  * Animation Clip のファイル名に拡張子 `.vrma` を付与する形で書き出し
  * 指定されたフォルダになんらかのファイルまたはフォルダが含まれても事前に警告を行いません
* `VRMC_vrm_animation` のメタデータに変換元の Animation Clip のファイル名と GUID が含まれる
* 変換元アバターのヒューマノイドにマッピングされているノードのみ書き出される
  * それ以外の無関係のノードは出力段階でフィルタリングされ、書き出し前に除去されます
  * ノード名は変換元アバターの元のノード名が使用されます
* 変換元アバターのメッシュとマテリアルはアニメーションに一切含まれない
* 表情のアニメーションは含まれない

[^1]: 1.0.5 以前は枝分かれに考慮されていなかったため、枝分かれも繋がったひとつのスプリングボーンとして出力されていました
[^2]: Position Constraint (実質的に Parent Constraint も同様) は未対応ですが https://github.com/vrm-c/vrm-specification/issues/468 で要望があがっています
[^3]: VRM Constraint が glTF のノードの拡張として実装されており、ひとつのノード（＝ゲームオブジェクト）につきひとつの VRM Constraint しか持つことができないためです
[^4]: MToon が UV0 以外のテクスチャ座標に対応していないためです
